...
AppendMCAR <- function(data, cols, varsMCAR){
  ...
  for(t in 1:length(cols)) {
    ...
    # Make final check for presence of NAs in column.
    checkNA <- sum(is.na(data[, variable]))
    # If 0 NAs are present..
    if(checkNA == 0){
      # Append variable name to varsMCAR.
      varsMCAR <- c(variable, varsMCAR)
    }
...
AverageCorr <- function(results, param, data, method, taxa, treeName = NULL) {
  
  # Function for averaging out correlation coefficients of the results from the Simpute functions.
  
  # results = list object containing correlation coefficients for numerical traits
  # param = best parameter based on results of AverageErrors function
  # data = original dataframe containing trait information
  # method = imputation method. One of "MeanMode", "KNN", "RF", or "MICE"
  # taxa = name of taxonomic group
  ...
  reps <- length(results)
  # Make a list to hold the average correlation coefficients and SEs for the vars.
  ...
  for(t in 1:length(vars)) {
    ...
    # If the method is KNN or RF...
    if(method == "KNN"|method == "RF"){
      
      # Create empty vector to hold the results for each replicate.
      ...
      for (i in 1:reps) { 
        ...
        # If method is RF..
        if(method == "RF"){
          # Name according to rownames so we can keep track of the parameter values for ntree.
      ...
      # Calculate the average corr across the replicates.
      dfRep$average <- rowMeans(dfRep)
      # Calculate the standard error.
      dfRep$SE <- apply(dfRep[, 1:reps], MARGIN = 1, FUN = std.error)
      
      # Identify parameter value that corresponds to trait.
      bestP <- param[[grep(trait, names(param))]]
      if(method == "KNN"){
        # Subset out the correlation coefficient value based on bestP (row index for KNN).
        avgCorr <- dfRep[bestP, "average"]
        # Subset out the corresponding SE.
        SE <- dfRep[bestP, "SE"] 
      } else if(method == "RF"){
        # Subset out the correlation coefficient value based on bestP (contained in row names for RF).
        avgCorr <- dfRep[which(rownames(dfRep) == bestP), "average"]
        # Subset out the corresponding SE.
        SE <- dfRep[which(rownames(dfRep) == bestP), "SE"] 
      }
      
    } else if(method == "MICE"){
      
      # Create a list to hold the results for each replicate.
      ...
      # Identify parameter value that corresponds to trait.
      bestP <- param[[grep(trait, names(param))]]
      # Remove Xs from rownames of dfRepAll.
      rownames(dfRepAll) <- as.numeric(gsub("X", "", rownames(dfRepAll)))
      # Subset out the correlation coefficient value based on bestP (contained in row names for RF).
      avgCorr <- dfRepAll[which(rownames(dfRepAll) == bestP), "average"]
      # Subset out the corresponding SE.
      SE <- dfRepAll[which(rownames(dfRepAll) == bestP), "SE"] 
      
    } 
    
    # Append to the list that holds results for the trait.
    AVGFinal[[t]] <- avgCorr
    SEFinal[[t]] <- SE
  ...
  dfCorr <- as.data.frame("MAR")
  colnames(dfCorr) <- "missingness_type"
  # Make a list to hold the dataframes.
  l_dfCorr <- lapply(1:length(vars), function(x) dfCorr)
  names(l_dfCorr) <- vars
  ...
  for(t in 1:length(vars)) {
    ...
    # Append the average corr and standard errors to the corresponding dataframe.
    dfTrait[, paste(method, "R", sep = "_")] <- corr
    ...
    if(is.null(treeName)){
      # Write dataframe to file according to name of taxa, trait, and imputation method.
      WriteErrorRates(dfTrait, fileName = paste(taxa, trait, method, "Corr.csv", sep = "_"))
    } else if(!is.null(treeName)){
      # Write dataframe to file according to name of taxa, trait, imputation method, and tree name.
      WriteErrorRates(dfTrait, fileName = paste(taxa, trait, method, treeName, "Corr.csv", sep = "_"))
    }
    
  }
}

AverageErrors <- function(results, data, cont, cat, method, taxa, paramTrack = F, treeName = NULL) {
  ...
  for(t in 1:length(vars)) {
    ...
    if(method == "MeanMode"|method == "KNN"|method == "RF"){
      ...
      for (i in 1:reps) { 
        ...
        # If method is RF..
        if(method == "RF"){
          # Name error rates according to rownames so we can keep track of the parameter values for ntree.
          names(missingCol) <- rownames(dfRes) ### *** 
        }
        # Append to repRates.
        repRates[[i]] <- missingCol
      ...
      if(paramTrack == T) {
        ...
        if(trait %in% cont){
          ...
          # Identify value of parameter that maximizes the ARI.
          bestParam <- which.max(dfRep$average)
        }
        
        # If method is RF..
        if(method == "RF"){
          # Get name of parameter value using rownames.
      ...
      if(method == "MeanMode") {
        ...
        if(trait %in% cont){
          ...
          # Identify which parameter value that maximizes the ARI.
          bestP <- which.max(dfRep$average)
        }
      ...
      if(trait %in% cont){
        ...
        # Which value of m minimizes the ARI for the trait?
        bestParam <- which.max(dfRepAll$average)
        
      }
      
      # Subset out the metric.
  ...
  # Create a dataframe that contains information about missingness levels.
  dfError <- as.data.frame("MAR")
  colnames(dfError) <- "missingness_type"
  ...
  if(paramTrack == T) {
    ...
    dfParam <- as.data.frame("MAR")
    colnames(dfParam) <- "missingness_type"
    # Make a list to hold the dataframes.
  ...
  for(t in 1:length(vars)) {
    ...
    # If trait is numerical...
    if(trait %in% cont) {
      # Append the average NMSE and standard errors to the corresponding dataframe.
      dfTrait[, paste(method, "NMSE", sep = "_")] <- rate
      dfTrait[, paste(method, "SE", sep = "_")] <- se
      # If trait is categorical...
    } else if(trait %in% cat){
      # Append the average ARI and standard errors to the corresponding dataframe.
      dfTrait[, paste(method, "ARI", sep = "_")] <- rate
      dfTrait[, paste(method, "SE", sep = "_")] <- se
    }
    ...
    if(is.null(treeName)){
      # Write dataframe to file according to name of taxa, trait, and imputation method.
      WriteErrorRates(dfTrait, fileName = paste(taxa, trait, method, "ErrorRates.csv", sep = "_"))
    } else if(!is.null(treeName)){
      # Write dataframe to file according to name of taxa, trait, imputation method, and tree name.
      WriteErrorRates(dfTrait, fileName = paste(taxa, trait, method, treeName, "ErrorRates.csv", sep = "_"))
    }
    # If parameter tracking is on...
    if(paramTrack == T) {
      ...
      if(is.null(treeName)){
        ...
        WriteErrorRates(dfParam, fileName = paste(taxa, trait, method, "Parameters.csv", sep = "_"))
      } else if (!is.null(treeName)){
        # Write dataframe to file according to name of taxa, imputation method, and missingness pattern.
        WriteErrorRates(dfParam, fileName = paste(taxa, trait, method, treeName, "Parameters.csv", sep = "_"))
      }
    }
  }
  
  if(method == "MeanMode"){
    # Assign finalParam as NULL.
    finalParam <- NULL
  }
  
  # Return finalParam (for use in AverageCorr function).
  return(finalParam)
}

BackTransform <- function(origData, tfData, missData = NULL, cols, type = "norm") {
...
ImputeKNN <- function(dfTrue, dfMissing, cols, cont, cat, k, predictors, phyImp = F, l_dfMissing = NULL){
  ...
  # Function for imputing missing values using the kNN() function in the "VIM" package for imputation. Returns a list of error rates (MSE for continuous traits and ARI for categorical traits) and correlation coefficients for each parameter value tested.
  ...
  # Create an empty list to hold the correlation coefficient results for each numeric trait and value of k. +++
  l_Corr <- CreateNamedList(listLength = length(cols), elementNames = cols)
  ...
  for(t in 1:length(cols)) {
    ...
    # Create a vector to hold the correlation coefficients for each value of k. +++
    paramCorr <- vector(mode = "numeric", length = length(neighbours))
    ...
    for(k in 1:length(neighbours)) {
      ...
      if(trait %in% cont) {
        ...
        corrCoef <- cor(dfError[, trait], dfError[, "Imputed_Value"])
        
        # If the trait is categorical...
      } else if(trait %in% cat){
        # Calculate the adjusted rand index. $$$
        errorRate <- adjustedRandIndex(dfError[, trait], dfError[, "Imputed_Value"])
        # Set corrCoef to NA. +++
        corrCoef <- NA
      ...
      # Name corrCoef according to the value of k. +++
      names(corrCoef) <- NN
      # Append results to the kth element in the list. +++
      paramCorr[[k]] <- corrCoef
      # Append dfImputedTrait to l_dfImputedTrait.
      l_dfImputedTrait[[k]] <- dfImputedTrait
    }
    
    # Append to the tth elements in the lists.
    l_ErrorParams[[t]] <- paramRes
    # Append paramCorr to l_Corr. +++
    l_Corr[[t]] <- paramCorr
    # Append l_dfImputedTrait to l_l_dfImputedTrait. +++
  ...
  # Subset l_Corr to only include numerical traits. +++
  l_Corr <- l_Corr[names(l_Corr) %in% cont]
  
  # Return list of error rates for each trait and parameter value and list of imputed dataframes for each parameter value. +++
  return(list(l_dfImputedParam = l_dfImputedParam, l_ErrorParams = l_ErrorParams, l_Corr = l_Corr))
  
}

ImputeMeanMode <- function(dfTrue, dfMissing, cols, cont, cat){
...
ImputeMICE <- function(dfTrue, dfMissing, dfOrig, cols, cont, cat, inter = NULL, mSets, matPredictors, seed){
  ...
  # dfTrue = dataframe containing true (known) observations
  # dfMissing = dataframe containing missing values and missingness indicator columns
  # dfOrig = dataframe containing original values prior to normalization (necessary for backtransforming)
  # cols = names of traits (column names)
  # cont = names of numerical traits
  # cat = names of categorical traits
  ...
  # Get the names of the missing columns as we will needs these later on.
  missingCols <- colnames(dfMissing)[grep(pattern = "_NA", x = colnames(dfMissing))]
  # Ensure dfOrig contains same species as dfTrue. +++
  dfOrig <- dfOrig[dfOrig$species_name %in% dfTrue$species_name, ]
  ...
  # Create an empty list to hold the correlation coefficient results for each numeric trait and value of m. +++
  l_Corr <- CreateNamedList(listLength = length(mSets), elementNames = mSets)
  ...
  for(m in 1:length(mSets)) {
    ...
    names(param) <- cols
    # Create a vector to hold the correlation coefficients for each trait.
    paramCorr <- vector(mode = "numeric", length = length(cols))
    # Name according to traits.
    names(paramCorr) <- cols
    ...
    for(t in 1:length(cols)) {
      ...
      if(trait %in% inter) {
        ...
        # Unscale the imputed values using sd and mean of original data. +++
        dfSubset[, impCol] <- Unscale(dfSubset[, impCol], scale = sd(dfOrig[, trait], na.rm = T), centre = mean(dfOrig[, trait], na.rm = T))
        # Round to the nearest whole number. +++
        dfSubset[, impCol] <- as.integer(round(dfSubset[, impCol]))
        # Scale once again so we can compare to the original scaled data. +++
        dfSubset[, impCol] <- scale(dfSubset[, impCol], scale = sd(dfOrig[, trait], na.rm = T), center = mean(dfOrig[, trait], na.rm = T))
      }
      ...
      if(trait %in% cont) {
        ...
        # Calculate the correlation coefficient. +++
        corrCoef <- cor(dfSubset[, trueCol], dfSubset[, impCol])
        
      } else if(trait %in% cat){
        # Calculate the adjusted rand index. $$$
        errorRate <- adjustedRandIndex(dfSubset[, impCol], dfSubset[, trueCol])
        # Set corrCoef to NA. +++
        corrCoef <- NA
      }
      # Name the errorRate according to trait.
      names(errorRate) <- trait
      # Append to the list. 
      param[[t]] <- errorRate
      # Name corrCoef according to trait. +++
      names(corrCoef) <- trait
      # Append to the list. +++
      paramCorr[[t]] <- corrCoef
    }
    
    # Append to the mth elements in the lists. +++
    l_dfImputedParam[[m]] <- dfImputed
    l_ErrorParams[[m]] <- param
    # Subset paramCorr to only include numerical traits. +++
    paramCorr <- paramCorr[names(paramCorr) %in% cont]
    # Append to mth element in the list. Ensure numerical format. +++
    l_Corr[[m]] <- paramCorr
  }
...
ImputeRF <- function(dfTrue, dfMissing, dfOrig, cols, cont, cat, inter = NULL, ntrees, predictors, phyImp = F, l_dfMissing = NULL, seed){
  ...
  for(t in 1:length(cols)) {
    ...
    # Create a vector to hold the correlation coefficients for each value of ntree.
    paramCorr <- vector(mode = "numeric", length = length(ntrees))
    # Name according to ntrees.
    names(paramCorr) <- ntrees
    ...
    for(n in 1:length(ntrees)) {
      ...
      if(trait %in% inter) {
        ...
        # Unscale the imputed values using sd and mean of original data. +++
        dfError$Imputed_Value <- Unscale(dfError$Imputed_Value, scale = sd(dfOrig[, trait], na.rm = T), centre = mean(dfOrig[, trait], na.rm = T))
        # Round to the nearest whole number. +++
        dfError$Imputed_Value <- as.integer(round(dfError$Imputed_Value))
        # Scale once again so we can compare to the original scaled data. +++
        dfError$Imputed_Value <- scale(dfError$Imputed_Value, scale = sd(dfOrig[, trait], na.rm = T), center = mean(dfOrig[, trait], na.rm = T))
      }
      # If the trait is numerical...
      if(trait %in% cont) {
      ...
      # Name corrCoef according to the value of ntree.
      names(corrCoef) <- N
      # Append corrCoef to the nth element in the lists. ^
      paramCorr[[n]] <- corrCoef
      # Append dfImputedTrait to l_dfImputedTrait.
