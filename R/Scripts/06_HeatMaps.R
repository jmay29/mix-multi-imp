...
# Identify file names.
errorFiles <- list.files(path = "Results/ErrorRates/", pattern = "_ErrorRates.csv", recursive = T)
# Read in error files.
...
# Remove class names from names of l_dfErrors.
names(l_dfErrors) <- gsub("Actinopterygii/|Amphibia/|Aves/|Mammalia/|Reptilia/", replacement = "", names(l_dfErrors))
